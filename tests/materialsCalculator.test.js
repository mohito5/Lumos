...
import {
  ...
  resolveMaterialDisplay,
  UNKNOWN_MATERIAL_PREFIX,
  unknownMaterialId,
} from '../src/shared/lib/materialsCalculator';
...
import { LEVEL_MILESTONES } from '../src/shared/config/constants';
...
describe('resolveMaterialDisplay', () => {
  // Регрессия: CharacterInfoPageView.jsx/WeaponInfoPageView.jsx раньше делали
  // materialsData.find(m => m.id === id) напрямую и молча пропускали строку
  // (return null), если id — синтетический __unknown_* (см. UNKNOWN_MATERIAL_
  // PREFIX) — материалы с неопределённой конкретной позицией пропадали из
  // списка вместо заглушки, которую та же ситуация корректно показывает на
  // странице возвышения через MaterialProgressCard.jsx.
  const fakeT = (key, opts) => {
    if (opts?.ns === 'materials' && key.startsWith('unknown.')) {
      const category = key.slice('unknown.'.length);
      return category === 'material' ? 'Неизвестный материал' : `Неизвестный (${category})`;
    }
    return opts?.defaultValue ?? key;
  };

  it('для настоящего id возвращает данные материала из materialsData', () => {
    const real = materialsData[0];
    const result = resolveMaterialDisplay(real.id, fakeT);
    expect(result).toMatchObject({ id: real.id, icon: real.icon, isUnknown: false });
  });

  it('для несуществующего (не __unknown_) id возвращает null — как и раньше', () => {
    expect(resolveMaterialDisplay('this_id_does_not_exist_anywhere', fakeT)).toBeNull();
  });

  it('для синтетического __unknown_* id возвращает плейсхолдер, а НЕ null', () => {
    const id = unknownMaterialId('ascension_material');
    const result = resolveMaterialDisplay(id, fakeT);
    expect(result).not.toBeNull();
    expect(result.isUnknown).toBe(true);
    expect(result.id).toBe(id);
    expect(result.icon).toBe('assets/unknown.png');
  });

  it('id действительно начинается с UNKNOWN_MATERIAL_PREFIX для любой категории из unknownMaterialId', () => {
    expect(unknownMaterialId('boss_material').startsWith(UNKNOWN_MATERIAL_PREFIX)).toBe(true);
  });

  it('переводит категорию в читаемое имя через переданный t()', () => {
    const result = resolveMaterialDisplay(unknownMaterialId('boss_material'), fakeT);
    expect(result.name).toBe('Неизвестный (boss_material)');
  });
});
