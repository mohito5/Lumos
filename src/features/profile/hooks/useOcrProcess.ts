...
export const useOcrProcess = ({
  ...
  const runOcr = useCallback(async (file: File): Promise<void> => {
    ...
    try {
      ...
      // ── 5. Готово ──────────────────────────────────────────────────────
      // Раньше onComplete/showNotification могли не выполниться вовсе,
      // если единственная ячейка попадала под один из `continue` выше —
      // теперь этот участок гарантированно достижим после цикла независимо
      // от того, сколько ячеек было пропущено.
      onComplete(result);
      const count = Object.keys(result).length;
      setStep(100, `Готово. Метод: ${method}. Найдено: ${count} видов материалов (ячеек обработано: ${slots.length}).`);
      showNotification(
        count > 0
          ? `Инвентарь отсканирован: ${count} видов материалов`
          : 'Сканирование завершено, материалы не найдены',
  ...
  }, [allMaterials, imgRef, onComplete, setStep]);

  const resetProgress = (): void => setProgress(INITIAL_PROGRESS);
