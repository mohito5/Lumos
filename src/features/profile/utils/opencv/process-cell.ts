...
//
// БЫЛО (прошлый аудит): здесь же жил CLEAN_NUMBER_PATTERN — второй, отдельный
// от digit-matching.ts, слой проверки валидности числа (тот отсекал только
// "?", этот — валидные-по-набору-символов, но странные по форме строки
// вроде нескольких точек подряд). Проблема была не в самой проверке, а в том,
// что логика "валидно ли число" была размазана по двум файлам — при правке
// одной легко забыть про другую. Теперь recognizeQuantityByTemplate сама
// возвращает isValid, здесь только читаем флаг.
...
export function processCell(
  ...
  try {
    ...
    // РАНЬШЕ: else всегда давал rarity=3 — то есть 1★ и 2★ предметы НИКОГДА
    // не могли получить свою настоящую редкость, только 3/4/5 были достижимы.
    // На removeBackground() это не влияло (1-3★ и раньше, и сейчас идут через
    // один и тот же GrabCut-путь — см. background-removal.ts, rarity там не
    // используется как число, только "≥4 или нет"), но rowLog.редкость врал
    // в диагностике. Оценка через HSV hue вместо частичных RGB-порогов —
    // ⚠️ калибровано по стандартной палитре звёзд Genshin (золото/фиолет/
    // синий/зелёный/серый), НЕ проверено на реальных 2★/3★ скриншотах — если
    // в rowLog редкость окажется систематически не той, поправь диапазоны hue
    // ниже (посмотри реальный hue через #/dev/digit-calibration или добавь
    // ocrLog.info с h/s/v сюда для конкретного скрина).
    let rarity = 1;
    if (starsMat) {
      const [r, g, b] = window.cv.mean(starsMat);
      ...
      if (saturation < 0.15) {
        rarity = 1; // низкая насыщенность = серо-белый
      } else {
        let hue = 0;
        if (delta > 0) {
        ...
        if (hue >= 20 && hue < 70) rarity = 5; // золото/оранжевый
        else if (hue >= 260 && hue < 320) rarity = 4; // фиолетовый
        else if (hue >= 180 && hue < 260) rarity = 3; // синий
        else if (hue >= 70 && hue < 180) rarity = 2; // зелёный
        else rarity = 1; // не попало ни в один диапазон — считаем невзрачным
      }
    }
    ...
    const { quantity, text, avgScore, isValid } = recognizeQuantityByTemplate(numMat, digitTemplateCache);
