...
// Форма итоговой строки распознавания: целое число (1-6 цифр) ЛИБО
// "k"-нотация (1-3 цифры, опционально 1-2 после точки, символ k/K).
// ПЕРЕНЕСЕНО СЮДА из process-cell.ts (было CLEAN_NUMBER_PATTERN) — валидность
// числа проверялась в двух местах (там — по этому паттерну, здесь — только
// отказ при "?"), и была реальная логика на два файла вместо одного. Теперь
// recognizeQuantityByTemplate — единственный источник истины "валидно ли
// распознанное количество", process-cell.ts просто читает флаг isValid.
 ...
 *
 * isValid=true означает ОБА условия сразу: ни один символ не отклонён по
 * score (нет "?" в text) И итоговая строка имеет форму, ожидаемую от
 * количества предмета (см. CLEAN_NUMBER_PATTERN) — например, несколько
 * точек подряд технически "все символы опознаны", но это не похоже на
 * настоящее число и должно быть отклонено так же, как и "?".
 */
export const recognizeQuantityByTemplate = (
  numMat: CvMat,
  templates: DigitTemplateCache,
): { text: string; quantity: number; avgScore: number; isValid: boolean } => {
  const empty = { text: '', quantity: 0, avgScore: 0, isValid: false };
  ...
  try {
    ...
    // Если хоть один символ не опознан ("?") — не гадаем, quantity=0.
    const hasUnrecognizedChar = text.includes('?');
    const finalQuantity = hasUnrecognizedChar ? 0 : quantity;

    return {
      text,
      quantity: finalQuantity,
      avgScore: scoreCount > 0 ? scoreSum / scoreCount : 0,
      isValid: finalQuantity > 0 && !hasUnrecognizedChar && CLEAN_NUMBER_PATTERN.test(text),
    };
  } catch (e) {
