import { showNotification, updateNotification, dismissNotification } from '../utils/notifications';
...
export const saveKeyToCloudChunked = async (key: CloudKey, payload: StoredPayload): Promise<boolean> => {
    ...
    if (json.length <= CHUNK_SAFE_LIMIT) {
        return setItemVerified(key, json);
    }
    ...
    return setItemVerified(key, JSON.stringify(manifest));
};
