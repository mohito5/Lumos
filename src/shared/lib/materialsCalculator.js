...
/**
 * Категория материала для placeholder-заглушки, когда в данных персонажа/
 * оружия не указан конкретный материал этой категории (см. UNKNOWN_MATERIAL_
 * PREFIX ниже). Определяется по type+group — этого достаточно, чтобы
 * различить "диковина" от "книги талантов" от "кристалла возвышения" и т.д.
 */
function describeMaterialCategory(cost) {
    if (cost.type === MATERIAL_TYPE.LOCAL_SPECIALTY) return 'local_specialty';
    if (cost.type === MATERIAL_TYPE.CHARACTER_TALENT) return 'talent_book';
    if (cost.type === MATERIAL_TYPE.CHARACTER_WEAPON_ENHANCEMENT) return 'enemy_drop';
    if (cost.type === MATERIAL_TYPE.WEAPON_ENHANCEMENT_MATERIALS) return 'weapon_material';
    if (cost.type === MATERIAL_TYPE.CHARACTER_ASCENTION) {
        if (cost.group === MATERIAL_GROUP.ASCENSION_GEMS || cost.group === MATERIAL_GROUP.COMMON_ASCENSION_GEMS) return 'ascension_gem';
        if (cost.group === MATERIAL_GROUP.WEEKLY_BOSS_DROPS || cost.group === MATERIAL_GROUP.NORMAL_BOSS_DROPS) return 'boss_material';
        return 'ascension_material';
    }
    return 'material';
}

/** Префикс синтетических id — заведомо не пересекается с реальными id материалов
 *  (те все — snake_case без ведущего __). MaterialProgressCard.jsx проверяет
 *  этот префикс, чтобы отрисовать иконку-заглушку + "Неизвестная ..." вместо
 *  обычного lookup по materialsData (там такого id не будет — и не должно). */
export const UNKNOWN_MATERIAL_PREFIX = '__unknown_';
export const unknownMaterialId = (category) => `${UNKNOWN_MATERIAL_PREFIX}${category}`;

/**
 * Резолвит materialId (настоящий ИЛИ синтетический __unknown_*) в данные для
 * отображения — единое место для той же логики, что уже реализована внутри
 * MaterialProgressCard.jsx (используется на странице возвышения). Раньше
 * страницы информации о персонаже/оружии (CharacterInfoPageView.jsx,
 * WeaponInfoPageView.jsx) делали `materialsData.find(m => m.id === id)` и
 * при !materialDetails просто пропускали строку (`return null`) — для
 * __unknown_* id это ВСЕГДА null, так что материалы с неизвестной категорией
 * молча пропадали из списка вместо заглушки, которую те же данные корректно
 * показывают на странице возвышения.
 *
 * @param {string} materialId
 * @param {(key: string, opts?: object) => string} t — i18next t(), с доступом к неймспейсу 'materials'
 * @returns {{ id: string, name: string, icon: string, rarity?: number, isUnknown: boolean } | null}
 */
export function resolveMaterialDisplay(materialId, t) {
    if (materialId.startsWith(UNKNOWN_MATERIAL_PREFIX)) {
        const category = materialId.slice(UNKNOWN_MATERIAL_PREFIX.length);
        return {
            id: materialId,
            name: t(`unknown.${category}`, { ns: 'materials', defaultValue: t('unknown.material', { ns: 'materials', defaultValue: 'Неизвестный материал' }) }),
    ...
    return { ...material, isUnknown: false };
}
...
export function calculateCharacterCostRange(type, from, to, character) {
    ...
    const processCost = (cost) => {
        ...
        // Раньше: если materialId не удавалось определить (чаще всего —
        // character.ascensionMaterials[cost.group] пуст в данных персонажа),
        // addMaterial(undefined, ...) молча ничего не делал — вся потребность
        // (например 30 диковин) исчезала из итога без следа, MaterialsGrid
        // просто не показывал эту карточку вообще. Теперь — placeholder-id
        // категории, MaterialProgressCard.jsx отрисует его как заглушку с
        // понятным названием ("Неизвестная диковина") вместо тишины.
        if (materialId) {
            addMaterial(materialId, cost.count);
        } else if (cost.group) {
            addMaterial(unknownMaterialId(describeMaterialCategory(cost)), cost.count);
        }
    };
    ...
    for (let i = from; i < to; i++) {
        ...
        for (const cost of costs) {
            ...
            if ((isWeaponAscensionMat || isEnemyDropMat) && cost.group) {
                ...
                const materialId = specificMaterialGroup ? findMaterialId(specificMaterialGroup, cost.rarity) : null;
                if (materialId) {
                    addMaterial(materialId, cost.count);
                } else {
                    // См. комментарий у аналогичного места в calculateCharacterCostRange —
                    // weapon.ascensionMaterials[cost.group] не заполнен в данных оружия.
                    addMaterial(unknownMaterialId(describeMaterialCategory(cost)), cost.count);
                }
            }
        }
